...
\**************************************/
Pcd8544_Status PCD8544_Init(Pcd8544_Handle* self) {
	Pcd8544_Status status = PCD8544_OK;

	// Control VCC if user provides the pin, otherwise assume external 3.3V
	if (self->VccPin != NULL) {
		self->Drivers->GpioWritePin(self->VccPin, HIGH);
		self->Drivers->Delay(10); // Wait for VCC to stabilize
	}

	// Reset sequence
	self->Drivers->GpioWritePin(self->ResPin, LOW);
	self->Drivers->Delay(1); // Min 100ns, 1ms is safe
	self->Drivers->GpioWritePin(self->ResPin, HIGH);
	self->Drivers->Delay(50); // Wait for internal reset

	// Send configuration commands
	self->Drivers->GpioWritePin(self->CsPin, LOW);
	self->Drivers->GpioWritePin(self->DcPin, LOW);

	uint8_t command = SET_EXTENDINST;
	status = self->Drivers->SpiTransmit(self->Spi, &command, 1, self->SpiTransmitTimeout);
	if (status != PCD8544_OK) {
		self->Drivers->GpioWritePin(self->CsPin, HIGH);
		if (self->VccPin != NULL)
			self->Drivers->GpioWritePin(self->VccPin, LOW);
		return status;
	}
...
\**************************************/
void PCD8544_Deinit(Pcd8544_Handle* self) {
	self->Drivers->GpioWritePin(self->CsPin, HIGH);
	self->Drivers->GpioWritePin(self->DcPin, LOW);
	if (self->LedPin != NULL)
		self->Drivers->GpioWritePin(self->LedPin, LOW);
	if (self->VccPin != NULL)
		self->Drivers->GpioWritePin(self->VccPin, LOW);
}
...
\**************************************/
void PCD8544_SetBacklight(Pcd8544_Handle* self, uint8_t isEnabled) {
	self->Drivers->GpioWritePin(self->LedPin, !isEnabled);
}
...
Pcd8544_Status PCD8544_SetSleepMode(Pcd8544_Handle* self, uint8_t isEnabled) {
	...
	status = self->Drivers->SpiTransmit(self->Spi, &isEnabled, 1, self->SpiTransmitTimeout);
	if (status != PCD8544_OK) {
		self->Drivers->GpioWritePin(self->CsPin, HIGH);
		return status;
	}
...
Pcd8544_Status PCD8544_SetDisplayMode(Pcd8544_Handle* self, uint8_t mode) {
	...
	status = self->Drivers->SpiTransmit(self->Spi, &mode, 1, self->SpiTransmitTimeout);
	if (status != PCD8544_OK) {
		self->Drivers->GpioWritePin(self->CsPin, HIGH);
		return status;
	}
...
\**************************************/
Pcd8544_Status PCD8544_SetTempCoeff(Pcd8544_Handle* self, uint8_t coefficient) {
	Pcd8544_Status status = PCD8544_OK;
	...
	status = self->Drivers->SpiTransmit(self->Spi, &coefficient, 1, self->SpiTransmitTimeout);
	if (status != PCD8544_OK) {
		self->Drivers->GpioWritePin(self->CsPin, HIGH);
		return status;
	}
...
\**************************************/
Pcd8544_Status PCD8544_SetContrast(Pcd8544_Handle* self, uint8_t contrast) {
	Pcd8544_Status status = PCD8544_OK;
...
\**************************************/
void PCD8544_TogglePixelColor(Pcd8544_Handle* self, uint8_t posX, uint8_t posY) {
	if (posX >= PCD8544_SCREEN_WIDTH || posY >= PCD8544_SCREEN_HEIGHT)
		return;

	uint16_t regIndex = posX + (posY / 8) * PCD8544_SCREEN_WIDTH;

	self->FrameBuffer[regIndex] ^= (1U << (posY % 8));
}
...
\**************************************/
void PCD8544_SetPixelColor(Pcd8544_Handle* self, uint8_t posX, uint8_t posY, uint8_t isBlack) {
	if (posX >= PCD8544_SCREEN_WIDTH || posY >= PCD8544_SCREEN_HEIGHT)
		return;
...
\**************************************/
uint8_t PCD8544_GetPixelColor(Pcd8544_Handle* self, uint8_t posX, uint8_t posY) {
	if (posX >= PCD8544_SCREEN_WIDTH || posY >= PCD8544_SCREEN_HEIGHT)
		return 0;

	uint16_t regIndex = posX + (posY / 8) * PCD8544_SCREEN_WIDTH;

	return ((self->FrameBuffer[regIndex] & (1U << (posY % 8))) >> (posY % 8));
}
...
\**************************************/
void PCD8544_FillScreenColor(Pcd8544_Handle* self, uint8_t isBlack) {
	isBlack = isBlack ? 0xFF : 0x00;
	for (uint16_t i = 0; i < PCD8544_SCREEN_SIZE; i++)
		self->FrameBuffer[i] = isBlack;
}
...
Pcd8544_Status PCD8544_UpdateScreen(Pcd8544_Handle* self) {
	...
	self->Drivers->GpioWritePin(self->DcPin, HIGH);
	status = self->Drivers->SpiTransmit(self->Spi, self->FrameBuffer, PCD8544_SCREEN_SIZE,
										self->SpiTransmitTimeout);
	self->Drivers->GpioWritePin(self->DcPin, LOW);
	if (status != PCD8544_OK) {
		self->Drivers->GpioWritePin(self->CsPin, HIGH);
		return status;
